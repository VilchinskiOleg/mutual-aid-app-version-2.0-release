...
keytool -keystore ./clientkeystore.jks -alias ${ALIAS_VALUE} -certreq -file ./cert-file -storepass ${KEY_STORE_PASSWORD}
...
keytool -keystore ./clientkeystore.jks -alias CARoot -import -file ./ca-cert.crt -storepass ${KEY_STORE_PASSWORD} -noprompt
keytool -keystore ./clientkeystore.jks -alias ${ALIAS_VALUE} -import -file ./cert-signed -storepass ${KEY_STORE_PASSWORD}

#  4. Generate the SSL trust store for the Kafka client:
# [*] will ask => if you trust that cert:
keytool -keystore ./clienttruststore.jks -alias CARoot -import -file ./ca-cert.crt -storepass ${TRUST_STORE_PASSWORD} -noprompt